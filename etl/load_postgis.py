...
def create_tables(engine, drop=False):
    with engine.begin() as conn:
        ...
        # Check if exists
        res = conn.execute(text("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'villages');"))
        exists = res.scalar()
        if exists and not drop:
            res = conn.execute(text("SELECT COUNT(*) FROM villages;"))
            count = res.scalar()
            if count > 0:
                print(f"Table 'villages' exists and has {count} rows. Skipping creation (use --drop to override).")
                return False
...
def load_data(engine):
    ...
    # Map geometry to postgis geom column
    gdf = gdf.rename(columns={"geometry": "geom"}).set_geometry("geom")
    
    gdf.to_postgis("villages", engine, if_exists="append", index=False, 
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text("CREATE INDEX IF NOT EXISTS idx_villages_geom ON villages USING GIST (geom);"))
        conn.execute(text("CREATE INDEX IF NOT EXISTS idx_villages_resolution ON villages (resolution);"))

    # Print summary
    with engine.connect() as conn:
        total_rows = conn.execute(text("SELECT COUNT(*) FROM villages;")).scalar()
        res_counts = conn.execute(text("SELECT resolution, COUNT(*) FROM villages GROUP BY resolution;")).fetchall()
        prov_counts = conn.execute(text("SELECT province, COUNT(*) FROM villages WHERE resolution='village' GROUP BY province;")).fetchall()
        size_res = conn.execute(text("SELECT pg_size_pretty(pg_total_relation_size('villages'));")).scalar()
...
def main():
    ...
    parser.add_argument("--drop", action="store_true", help="Drop and recreate tables if they exist.")
