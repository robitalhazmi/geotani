...
if [ "$RECORD_COUNT" = "0" ] || [ -z "$RECORD_COUNT" ]; then
    echo "   Database tables not found. Automatically running ETL & scoring pipeline from scratch..."
    docker compose --env-file "$ENV_FILE" -f docker-compose.prod.yml run --rm api ./scripts/run_etl_pipeline.sh
    echo "   ✓ ETL data pipeline complete and PostGIS loaded."
else
